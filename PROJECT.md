...
- `tools.generate.image.defaults` / `profiles`: optional global and per-model image generation parameter defaults (`size`, `aspectRatio`, `maxRetries`, provider `options`) plus `useWhen` guidance surfaced in the tool description.
