...
Workflow JavaScript has no unsandboxed fallback. The Docker image is workflow-ready when run with the repository's systemd-PID1 Compose contract: Linux, Docker 28+, cgroup v2, a private writable cgroup namespace, and the documented unconfined seccomp/system-path options. It does not require privileged mode or a host cgroup bind. The image provisions Bubblewrap and a reachable `lilac` user systemd manager with delegated memory/PID controls; `bun run docker:verify-image` verifies the built image without credentials, and missing dependencies still fail workflow-engine startup closed with an actionable error. See `docs/docker-deployment.md`.
