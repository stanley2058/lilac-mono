# Issue tracker

This repository tracks open and deferred work in local Markdown files.

- Read [plans and confirmed residuals](../../plan/README.md) for outstanding work and its approval status.
- For review, use the relevant issue's acceptance criteria as the spec. For completed work, use Git history to recover the issue version that governed the reviewed changes, before the document was removed.
- Remove completed issues after fixes and verification are committed. Delete empty trackers and update affected links. Preserve current architecture decisions in [PROJECT.md](../../PROJECT.md) and migration guidance in [MIGRATIONS.md](../../MIGRATIONS.md).
