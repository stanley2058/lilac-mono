...
import { mergeByDiscordWindow } from "./merge-window";
import {
...
export class DiscordAdapter implements SurfaceAdapter {
  ...
  private async onMessageCreate(msg: Message | PartialMessage) {
    ...
    // Build merged text window (same author within 7 min)
    const recent = store.listMessages(channelId, 50);
    const candidates = recent
      .filter((r) => r.deleted_ts == null)
      .map((r) => ({
    ...
    // Ensure the triggering message is present; it should be, but be defensive.
    if (!candidates.some((c) => c.messageId === msg.id)) {
      candidates.unshift({
        messageId: msg.id,
        authorId: msg.author.id,
        ts,
    ...
    // candidates are newest->oldest from listMessages
    const merged = mergeByDiscordWindow(candidates);

    let mergedText = merged.mergedText;
    if (triggerType === "mention") {
      // Strip leading mention only on the first chunk; mergedText begins with oldest.
      mergedText = stripLeadingBotMention({
        text: mergedText,
        botUserId: botId,
      });
    }
    ...
    this.emit({
      ...
      raw: {
        triggerType,
        mergedMessageIds: merged.mergedMessageIds,
        session: {
          channelId,
          guildId,
          parentChannelId,
