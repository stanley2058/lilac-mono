/**
 * Applies the OpenAI Responses apply_patch "diff" format.
 *
 * This is copied (with light adaptation) from the AI SDK examples:
 * `ref/ai/examples/ai-functions/src/lib/apply-diff.ts`.
 *
 * Notes:
 * - The diff is "headerless" and uses sections separated by `@@` anchors.
 * - Lines are prefixed with one of: ' ' (keep), '-' (delete), '+' (insert).
 * - "create" mode requires all lines to start with '+'.
 */

export function applyOpenAIApplyPatchDiff(
  input: string,
  diff: string,
...
function parseCreateDiff(lines: string[]): string {
  ...
  while (!isDone(parser, SECTION_TERMINATORS)) {
    ...
    if (line === undefined) {
      throw new Error("Invalid diff: unexpected end while creating file");
    }
    parser.index += 1;
    if (!line.startsWith("+")) {
...
function parseUpdateDiff(
  ...
  while (!isDone(parser, END_SECTION_MARKERS)) {
    ...
    const hasBareAnchor =
      !anchor && parser.lines[parser.index] !== undefined && parser.lines[parser.index] === "@@";
...
function equalsSlice(
  ...
  for (let i = 0; i < target.length; i += 1) {
    const src = source[start + i];
    const dst = target[i];
    if (src === undefined || dst === undefined) return false;
    if (mapFn(src) !== mapFn(dst)) return false;
  }
  return true;
}
