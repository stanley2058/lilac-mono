...
export const imageGenerateInputSchema = z
  .object({
    ...
    model: z
      ...
      .optional()
      .describe(
        "Image model to use. If omitted, picks first configured model in fallback order.",
      ),
...
function validateNanobananaInput(input: ImageGenerateInput, modelId: "nanobanana" | "nanobanana-pro"): void {
...
export class Generate implements ServerTool {
  ...
  private async callGenerateImage(
    ...
    const aspectRatio =
      !payload.size && payload.aspectRatio ? (payload.aspectRatio as `${number}:${number}`) : undefined;
    ...
    const targetWithExt = originalExt.length > 0 ? resolvedTarget : `${resolvedTarget}${inferredExt}`;
