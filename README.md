...
Lilac is an event-driven agent runtime: a typed Redis Streams event bus + a surface adapter (Discord) + an agent runner (AI SDK) + an HTTP tool server and `tools` CLI.
...
- `apps/core/`: core runtime (Discord adapter, bus wiring, router, agent runner, workflow, tool server)
